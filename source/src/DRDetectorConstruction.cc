 ...
 *	Member Functions
 */
DRDetectorConstruction::~DRDetectorConstruction()
{
...
//
DRDetectorConstruction::DRDetectorConstruction(RunParameters params)
{
...
//	MyConstructor
//
void DRDetectorConstruction::MyConstructor(RunParameters params)
{
	runParams = params;
...
//	mQuartz = new G4Material("Quartz", 2.2*g/cm3, 2);
//	mQuartz->AddElement(eO, 2);
//	mQuartz->AddElement(eSi)
	...
	const G4int nEntries_SiO2 = 2;
	G4double PHOTONENERGY_SiO2[nEntries_SiO2] = {1.*eV, 12.*eV};
	G4double RINDEX_SiO2[nEntries_SiO2] = {1.71, 1.71};
	mptSiO2->AddProperty("RINDEX", PHOTONENERGY_SiO2, RINDEX_SiO2, 
			nEntries_SiO2);
...
*/
	...
	mSiO2->SetMaterialPropertiesTable(mptSiO2);
...
//	G4 Method
//
G4VPhysicalVolume* DRDetectorConstruction::Construct()
{
...
//
G4VPhysicalVolume* DRDetectorConstruction::BuildGeometry()
{
...
//
//	Build Dual Readout Module
//
void DRDetectorConstruction::BuildDR()
{
	//
	//	SD
	//
	G4SDManager *SDManager = G4SDManager::GetSDMpointer();
	DRSD *SD_EM_S = new DRSD("SD_EM_S", runParams.vTrees[0], runParams);
	DRSD *SD_EM_C = new DRSD("SD_EM_C", runParams.vTrees[1], runParams);
	DRSD *SD_HAD_S = new DRSD("SD_HAD_S", runParams.vTrees[2], runParams);
	DRSD *SD_HAD_C = new DRSD("SD_HAD_C", runParams.vTrees[3], runParams);
	SDManager->AddNewDetector(SD_EM_S);
	SDManager->AddNewDetector(SD_EM_C);
	SDManager->AddNewDetector(SD_HAD_S);
	SDManager->AddNewDetector(SD_HAD_C);

	ReadDRConfigFile();
	PrintDRConfigParameters();

	//
	//	Define additional Dimensions
	//
	G4double fullEMScintX = 50.*cm;
	G4double fullEMScintY = 50.*cm;

	G4double fullEMCerX = 50.*cm;
	G4double fullEMCerY = 50.*cm;

	G4double fullEMAbsX = 50.*cm;
	G4double fullEMAbsY = 50.*cm;

	G4double fullEMLayerX = fullEMAbsX;
	G4double fullEMLayerY = fullEMAbsY;
	G4double fullEMLayerZ = drParameters.fullEMAbsZ + 
		drParameters.fullEMScintZ +	drParameters.fullEMCerZ;

	G4double fullDREMX = fullEMLayerX;
	G4double fullDREMY = fullEMLayerY;
	G4double fullDREMZ = drParameters.numEMLayers*fullEMLayerZ;

	G4double fullHADScintX = 50.*cm;
	G4double fullHADScintY = 50.*cm;

	G4double fullHADCerX = 50.*cm;
	G4double fullHADCerY = 50.*cm;

	G4double fullHADAbsX = 50.*cm;
	G4double fullHADAbsY = 50.*cm;

	G4double fullHADLayerX = fullHADAbsX;
	G4double fullHADLayerY = fullHADAbsY;
	G4double fullHADLayerZ = drParameters.fullHADAbsZ + 
		drParameters.fullHADScintZ + drParameters.fullHADCerZ;

	G4double fullDRHADX = fullHADLayerX;
	G4double fullDRHADY = fullHADLayerY;
	G4double fullDRHADZ = drParameters.numHADLayers*fullHADLayerZ;

	G4double fullDRX = fullDRHADX;
	G4double fullDRY = fullDRHADY;
	G4double fullDRZ = fullDRHADZ + fullDREMZ;

	G4cout << "### EM Dimensions Summary: ###" << endl
		<< "######################################" << endl
		<< "fullEMLayerX=" << fullEMLayerX/mm << endl
		<< "fullEMLayerY=" << fullEMLayerY/mm << endl
		<< "fullEMLayerZ=" << fullEMLayerZ/mm << endl
		<< "fullEMScintX=" << fullEMScintX/mm << endl
		<< "fullEMScintY=" << fullEMScintY/mm << endl
		<< "fullEMScintZ=" << drParameters.fullEMScintZ/mm << endl
		<< "fullEMCerX=" << fullEMCerX/mm << endl
		<< "fullEMCerY=" << fullEMCerY/mm << endl
		<< "fullEMCerZ=" << drParameters.fullEMCerZ/mm << endl
		<< "fullEMAbsX=" << fullEMAbsX/mm << endl
		<< "fullEMAbsY=" << fullEMAbsY/mm << endl
		<< "fullEMAbsZ=" << drParameters.fullEMAbsZ/mm << endl
		<< "fullDREMZ=" << fullDREMZ << endl;
	G4cout << "### HAD Dimensions Summary: ###" << endl
		<< "######################################" << endl
		<< "fullHADLayerX=" << fullHADLayerX/mm << endl
		<< "fullHADLayerY=" << fullHADLayerY/mm << endl
		<< "fullHADLayerZ=" << fullHADLayerZ/mm << endl
		<< "fullHADScintX=" << fullHADScintX/mm << endl
		<< "fullHADScintY=" << fullHADScintY/mm << endl
		<< "fullHADScintZ=" << drParameters.fullHADScintZ/mm << endl
		<< "fullHADCerX=" << fullHADCerX/mm << endl
		<< "fullHADCerY=" << fullHADCerY/mm << endl
		<< "fullHADCerZ=" << drParameters.fullHADCerZ/mm << endl
		<< "fullHADAbsX=" << fullHADAbsX/mm << endl
		<< "fullHADAbsY=" << fullHADAbsY/mm << endl
		<< "fullHADAbsZ=" << drParameters.fullHADAbsZ/mm << endl
		<< "fullDRHADZ=" << fullDRHADZ/mm << endl;

	//
	//	The full DR(EM + HAD) Module
	//
	solidDR = new G4Box("solidDR", fullDRX/2., fullDRY/2., fullDRZ/2.);
	logicDR = new G4LogicalVolume(solidDR, mVacuum, "logicDR");
	physDR = new G4PVPlacement(0, G4ThreeVector(), logicDR, "physDR",
			logicWorld, 0, 0, true);

	//
	//	DR EM Container
	//
	G4double zpos = -fullDRZ/2. + fullDREMZ/2.;
	solidDREM = new G4Box("solidDREM", fullDREMX/2., 
			fullDREMY/2., fullDREMZ/2.);
	logicDREM = new G4LogicalVolume(solidDREM, mVacuum, "logicDREM");

	if (drParameters.emOnOff == 1)
		physDREM = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos), 
			logicDREM, "physDREM", logicDR, 0, 0, true);

	//
	//	DR HAD Container
	//
	zpos = -fullDRZ/2. + fullDREMZ + fullDRHADZ/2.;
	solidDRHAD = new G4Box("solidDRHAD", fullDRHADX/2., 
			fullDRHADY/2., fullDRHADZ/2.);
	logicDRHAD = new G4LogicalVolume(solidDRHAD, mVacuum, "logicDRHAD");
	physDRHAD = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos), 
			logicDRHAD, "physDRHAD", logicDR, 0, 0, true);

	//
	//	EM Layers
	//
	solidDREMLayer = new G4Box("solidDREMLayer", fullEMLayerX/2., 
			fullEMLayerY/2., fullEMLayerZ/2.);
	logicDREMLayer = new G4LogicalVolume(solidDREMLayer, mVacuum, 
			"logicDREMLayer");
	for (int iLayer=0; iLayer<drParameters.numEMLayers; iLayer++)
	{
		zpos = -fullDREMZ/2. + (iLayer + 0.5)*fullEMLayerZ;
		physDREMLayer = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos),
				logicDREMLayer, "physDREMLayer", logicDREM,
				0, iLayer, true);
	}
	...
	//	Put HAD Layers
	//
	solidDRHADLayer = new G4Box("solidDRHADLayer", fullHADLayerX/2., 
			fullHADLayerY/2., fullHADLayerZ/2.);
	logicDRHADLayer = new G4LogicalVolume(solidDRHADLayer, mVacuum, 
			"logicDRHADLayer");
	for (int iLayer=0; iLayer<drParameters.numHADLayers; iLayer++)
	{
		zpos = -fullDRHADZ/2. + (iLayer + 0.5)*fullHADLayerZ;
		physDRHADLayer = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos),
				logicDRHADLayer, "physDRHADLayer", logicDRHAD,
				0, iLayer, true);
	}
	...
	//	F -> B = S -> C -> Abs
	//
	solidDREMScintPart = new G4Box("solidDREMScintPart", fullEMScintX/2., 
			fullEMScintY/2., drParameters.fullEMScintZ/2.);
	logicDREMScintPart = new G4LogicalVolume(solidDREMScintPart, 
			mLYSO, "logicDREMScintPart", 0, SD_EM_S, 0);
	zpos = -fullEMLayerZ/2. + drParameters.fullEMScintZ/2.;
	physDREMScintPart = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos),
			logicDREMScintPart, "physDREMScintPart", logicDREMLayer,
			0, 0, true);

	solidDREMCerPart = new G4Box("solidDREMCerPart", fullEMCerX/2.,
			fullEMCerY/2., drParameters.fullEMCerZ/2.);
	logicDREMCerPart = new G4LogicalVolume(solidDREMCerPart,
			mSiO2, "logicDREMCerPart", 0, SD_EM_C, 0);
	zpos = -fullEMLayerZ/2. + drParameters.fullEMScintZ + 
		drParameters.fullEMCerZ/2;
	physDREMCerPart = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos),
			logicDREMCerPart, "physDREMCerPart", logicDREMLayer, 0, 0, true);

	solidDREMAbs = new G4Box("solidDREMAbs", fullEMAbsX/2., fullEMAbsY/2.,
			drParameters.fullEMAbsZ/2.);
	logicDREMAbs = new G4LogicalVolume(solidDREMAbs, drParameters.mEMAbs,
			"logicDREMAbs");
	zpos = -fullEMLayerZ/2. + drParameters.fullEMScintZ + 
		drParameters.fullEMCerZ + drParameters.fullEMAbsZ/2.;
	physDREMAbs = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos),
			logicDREMAbs, "physDREMAbs", logicDREMLayer, 0, 0, true);

	//	
	//	HAD Layer composites:
	//	F -> B = S -> C -> Abs
	//
	solidDRHADScintPart = new G4Box("solidDRHADScintPart", fullHADScintX/2., 
			fullHADScintY/2., drParameters.fullHADScintZ/2.);
	logicDRHADScintPart = new G4LogicalVolume(solidDRHADScintPart, 
			mLYSO, "logicDRHADScintPart", 0, SD_HAD_S, 0);
	zpos = -fullHADLayerZ/2. + drParameters.fullHADScintZ/2;
	physDRHADScintPart = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos),
			logicDRHADScintPart, "physDRHADScintPart", logicDRHADLayer,
			0, 0, true);

	solidDRHADCerPart = new G4Box("solidDRHADCerPart", fullHADCerX/2.,
			fullHADCerY/2., drParameters.fullHADCerZ/2.);
	logicDRHADCerPart = new G4LogicalVolume(solidDRHADCerPart,
			mSiO2, "logicDRHADCerPart", 0, SD_HAD_C, 0);
	zpos = -fullHADLayerZ/2. + drParameters.fullHADScintZ + 
		drParameters.fullHADCerZ/2;
	physDRHADCerPart = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos),
			logicDRHADCerPart, "physDRHADCerPart", logicDRHADLayer, 
			0, 0, true);

	solidDRHADAbs = new G4Box("solidDRHADAbs", fullHADAbsX/2., fullHADAbsY/2.,
			drParameters.fullHADAbsZ/2.);
	logicDRHADAbs = new G4LogicalVolume(solidDRHADAbs, drParameters.mHADAbs,
			"logicDRHADAbs");
	zpos = -fullHADLayerZ/2. + drParameters.fullHADScintZ + 
		drParameters.fullHADCerZ + drParameters.fullHADAbsZ/2.;
	physDRHADAbs = new G4PVPlacement(0, G4ThreeVector(0, 0, zpos),
			logicDRHADAbs, "physDRHADAbs", logicDRHADLayer, 0, 0, true);

	return;
}

//
//	Input Parameters for DR Module and Configure them
//	
int DRDetectorConstruction::ReadDRConfigFile()
{
	cout << "### Reading in DualReadout Configuration File..." << endl;

	ifstream drConfigFile(runParams.drInputFileName.c_str());
	if (!drConfigFile)
	{
		cout << "### ERROR: File " << runParams.drInputFileName
	...
	double x,y,z;
	int onOff;
	int iEMMat, iHADMat, numEMLayers, numHADLayers;
	
	//
	//	Input EM Parameters
	//
	drConfigFile >> z;
	drParameters.fullEMScintZ = z*mm;
	drConfigFile >> z;
	drParameters.fullEMCerZ = z*mm;
	drConfigFile >> z;
	drParameters.fullEMAbsZ = z*mm;
	drConfigFile >> iEMMat;

	if (iEMMat==0)
		drParameters.mEMAbs = mCu;
	else
		drParameters.mEMAbs = mW;

	drConfigFile >> numEMLayers;
	drParameters.numEMLayers = numEMLayers;
	drConfigFile >> onOff;
	drParameters.emOnOff = onOff;

	//
	//	Input HAD
	//
	drConfigFile >> z;
	drParameters.fullHADScintZ = z*mm;
	drConfigFile >> z;
	drParameters.fullHADCerZ = z*mm;
	drConfigFile >> z;
	drParameters.fullHADAbsZ = z*mm;
	drConfigFile >> iHADMat;
	if (iHADMat==0)
		drParameters.mHADAbs = mCu;
	else
		drParameters.mHADAbs = mW;
	drConfigFile >> numHADLayers;
	drParameters.numHADLayers = numHADLayers;

	return 0;
}

void DRDetectorConstruction::PrintDRConfigParameters()
{
	cout << "### Dual Readout Parameters: ###" << endl;
	cout << "################################" << endl;

	G4cout << "### EM: " << "OnOff= " << drParameters.emOnOff << " ###" << endl;
	G4cout << "mEMAbs=" << drParameters.mEMAbs << endl;
	G4cout << "fullEMAbsZ=" << drParameters.fullEMAbsZ/mm << endl;
	G4cout << "fullEMScintZ=" << drParameters.fullEMScintZ/mm << endl;
	G4cout << "fullEMCerZ=" << drParameters.fullEMCerZ/mm << endl;
	G4cout << "numEMLayers=" << drParameters.numEMLayers << endl;
	G4cout << "### HAD: ###" << endl;
	G4cout << "mHADAbs=" << drParameters.mHADAbs << endl;
	G4cout << "fullHADAbsZ=" << drParameters.fullHADAbsZ/mm << endl;
	G4cout << "fullHADScintZ=" << drParameters.fullHADScintZ/mm << endl;
	G4cout << "fullHADCerZ=" << drParameters.fullHADCerZ/mm << endl;
	G4cout << "numHADLayers=" << drParameters.numHADLayers << endl;
	G4cout << "################################" << endl;
 ...
 */
/*
void DetectorConstruction::BuildShashlik()
{
	//	SD
	//
	G4SDManager *SDManager = G4SDManager::GetSDMpointer();
